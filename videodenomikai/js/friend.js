...
function friends(){
	...
	$.ajax({
		...
		success:function(data,dataType){
			...
			//何人表示するかの話
				if(i<30){
					friendlist = friendlist+"<li><div  class='日本語トーク' id='"+friendArray[i+1]+"'  onClick='getProfile(this)'><img src='prof/"+friendArray[i]+"'/><br/>"+friendArray[i+2]+"</div></li>";

				}
				//もっと見るのための処理
				moreFriend = moreFriend + "<div style='display:inline-block;padding:8px 4px;cursor:pointer' id='"+friendArray[i+1]+"' class='日本語トーク' onClick='getProfile(this)'><img  style='width:100px' src='english/prof/"+friendArray[i]+"'/><br/>"+friendArray[i+2]+"</div>";
				if(i==brtriger){
	...
	$.ajax({
		...
		success:function(data,dataType){
			...
			for(var i=0;i<histArray.length-1;i=i+5){
				...
				//何人表示するかの話
				if(i<30){
					hist = hist+"<li><div id='"+histArray[i+2]+"' class='"+typeTmp+"' onClick='getProfile(this)'><img src='english/prof/"+histArray[i+1]+"'/><br/>"+histArray[i+3]+" さん<br/>"+histDate[0]+" "+histDate[1]+"<br/>"+typeTmp+"</div></li>";
				}
				//もっと見るのための処理
				moreHist = moreHist + "<div style='display:inline-block;padding:8px 4px;cursor:pointer' id='"+histArray[i+2]+"' class='"+typeTmp+"' onClick='getProfile(this)'><img  style='width:100px' src='english/prof/"+histArray[i+1]+"'/><br/>"+histArray[i+3]+" さん<br/>"+histDate[0]+" "+histDate[1]+"<br/>"+typeTmp+"</div>";
				if(i==brtriger){
