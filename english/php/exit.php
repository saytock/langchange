...
if(isset($_POST["break"])){
	...
	while($rows=$query->fetch(PDO::FETCH_ASSOC)){
		...
		//echo $error[2];
			//退会通知（ポップアップ）
			$pdo->query("UPDATE learning SET comment = 'j' WHERE userid = '".$rows["friendid"]."' && learningyear = '".$rows["teachingyear"]."' && learning = '".$rows["teaching"]."'");
			//退会通知（メール）
			$tTime = explode("=",$rows["teaching"]);
			$to = $rows["mail"];
			$title = "Your talk is canceled. -Langchange";
			$content = "Talk info: ".$tTime[0].", ".$tTime[1]."\n\n Your talk above is canceled because your partner ".$_SESSION["name"]." left Langchange.\nOpen the talk again to get a new partner.\n\n**Main Page URL: http://langchange.org/english/mainpage.html\n\n**Do not reply to this email. Thank you.";
	...
	while($rows=$query->fetch(PDO::FETCH_ASSOC)){
		...
		if($currentTime<$targetTime){
			//退会通知（ポップアップ）
			$pdo->query("UPDATE teaching SET checks='t' WHERE userid = '".$rows["friendid"]."' && teachingyear = '".$rows["learningyear"]."' && teaching = '".$rows["learning"]."'");
			//退会通知（メール）
			$tTime = explode("=",$rows["learning"]);
			$to = $rows["mail"];
			$title = "Your talk is canceled. -Langchange";
			$content = "Talk info: ".$tTime[0].", ".$tTime[1]."\n\nYour talk above is canceled because your partner ".$_SESSION["name"]." left Langchange.\nThe LC coin you used is returned.\n\n**Main Page URL: http://langchange.org/english/login.html\n\n**Do not reply to this email. Thank you.";
	$from = "From: Langchange<lc_info@langchange.org>";
	mb_language("Japanese");
	mb_internal_encoding("UTF-8");
	mb_send_mail($to, $title, $content, $from);
