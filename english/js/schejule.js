...
var sp=0;
	...
	for(var i=0;i<6;i=i+1){
		...
		if(2<sp)
		table = table + "<th class='no-sp'>"+String(month)+"/"+String(dateTmp)+"("+String(youbi[tmp])+")</th>";
		else	
		table = table + "<th>"+String(month)+"/"+String(dateTmp)+"("+String(youbi[tmp])+")</th>";
	...
	for(var a = 0;a<24;a=a+1){
		...
		for(var b = 0;b<6;b=b+1){
			...
			if(b==0&&cTime>=tmpTime){ //どの時間までインデックスをつけるか
				...
				table = table + "<td class ='sche-gradation'><div class='talk-fail' id='"+resistdate+"'><span style='color:#000' >not avalable</span></div></td>";
				grdTime = grdTime -1;
			}
		...
		for(var b = 0;b<6;b=b+1){
			...
			if(b==0&&cTime>tmpTime){ //どの時間までインデックスをつけるか
				...
				resistdate = String(tmpMonth)+"月"+tmpDate+"日="+rTime+":00";
				table = table + "<td class ='sche-gradation'><div class='talk-fail' id='"+resistdate+"'><span style='color:#000' >not available</span></div></td>";
				grdTime = grdTime -1;
