...
function friends(){
	...
	$.ajax({
		...
		success:function(data,dataType){
			...
			//何人表示するかの話
				if(i<18){
					friendlist = friendlist+"<li><div style='margin-top:15px' class='日本語トーク' id='"+friendArray[i+1]+"'  onClick='getProfile(this)'><span class='triming'><img src='../prof/"+friendArray[i]+"'/></span><br/>"+friendArray[i+2]+"</div></li>";

				}
				//もっと見るのための処理
				if(i<108){
				moreFriend = moreFriend + "<div style='display:inline-block;padding:8px 4px;cursor:pointer' id='"+friendArray[i+1]+"' class='日本語トーク' onClick='getProfile(this)'><span class='triming-prof'><img src='../prof/"+friendArray[i]+"'/></span><br/>"+friendArray[i+2]+"</div>";
				if(i==brtriger){
	...
	$.ajax({
		...
		success:function(data,dataType){
			...
			for(var i=0;i<histArray.length-1;i=i+5){
				...
				var apple = false; ////変更：英語トークもプロフィール表示可能に
				if(histArray[i+4]=="learning" && apple){
					typeTmp = "英語トーク";
					typeTmp2= "Talk in English";
					//何人表示するかの話
				if(i<30){
					hist = hist+"<li><div style='cursor:pointer' id='"+histArray[i+2]+"' class='"+typeTmp+"' onClick='getProfile(this)'><span class='triming'><img src='../prof/"+histArray[i+1]+"'/></span><br/>"+histArray[i+3]+"<br/>"+tmonth+"/"+tdate+" "+histDate[1]+"<br/>"+typeTmp2+"</div></li>";
				}
				//もっと見るのための処理
				if(i<180){
				moreHist = moreHist + "<div style='display:inline-block;padding:8px 4px;cursor:pointer' id='"+histArray[i+2]+"' class='"+typeTmp+"' onClick='getProfile(this)'><span class='triming-prof'><img src='../prof/"+histArray[i+1]+"'/></span><br/>"+histArray[i+3]+"<br/>"+tmonth+"/"+tdate+" "+histDate[1]+"<br/>"+typeTmp2+"</div>";
				if(i==brtriger){
