...
function initialize(){
	...
	$.ajax({
		...
		success:function(data,dataType){
			...
			//検索可能回数を表示
			try{
				document.getElementById("serching-count-sp").innerHTML = "Refine search: "+init[flag]+" times available <button class='addsearch' onClick='addSerching()'>Get more</button>";
			}catch(e){};
			try{
				document.getElementById("serching-count").innerHTML = "Refine search: "+init[flag]+" times available <button class='addsearch' onClick='addSerching()'>Get more</button>";
			}catch(e){};
...
openConfirm('Use 1 LC coin to book the talk?<br/><br/>**You can cancel it by clicking the button again, but the LC coin will not be returned.',function(cancel){
	if(cancel){
		return false;
	}else{
	//検索に関する処理
	...
	$.ajax({
		...
		error:function(XMLHttpRequest,textStatus,errorThrown){
				openDialog('The talk is not booked.');
...
if(checkSerching()){
	...
	openDialog("Click 'Search' button first. And then set the conditions.");
	document.getElementById("sex").value="all";
}
...
openConfirm("Start refine search?<br><br>**The effect will last until you book a talk.",
function(cancel){
	if(cancel){
		return false;
	}else{
